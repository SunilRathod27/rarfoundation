...
module.exports = {
   getBloodGroupList: async function (req, res) {
        try {
            let List = await RAR.App.Services.BloodGroup.SrvList.getBloodGroupList();
            if (List.statusCode == 200) {
                ...
                let obj = {
                    statusCode: 400,
                    message: "Error while get BloodGroup  list !",
                    result: List.message
                };
            ...
            console.log(" Error while get BloodGroup  list " + error.message);
            let obj = {
                statusCode: 400,
                message: " Error while get BloodGroup  list ",
                result: null
    ...
    addBloodGroup: async function (req, res) {
        try {
            let addBloodGroup = await RAR.App.Services.BloodGroup.SrvList.addBloodGroup(req.body);
            res.send(addBloodGroup);
        } catch (error) {
            let obj = {
                statusCode: 400,
                message: "Erro while add new BloodGroup",
                result: null
            };
            res.send(obj);
        }
    },
    editBloodGroup: async function (req, res) {
        try {
            let editBloodGroup = await RAR.App.Services.BloodGroup.SrvList.editBloodGroup(req.params.id, req.body);

            res.send(editBloodGroup);
        } catch (error) {
            console.log("Error While Update BloodGroup Con!! " + error.message);
            let obj = {
                statusCode: 400,

                message: "Error While Update BloodGroup",
                result: null
            };
            res.send(obj);
        }
    ...
    deleteBloodGroup: async function (req, res) {
        try {
            let deleteBloodGroup = await RAR.App.Services.BloodGroup.SrvList.deleteBloodGroup(req.params.id, req.body);

            res.send(deleteBloodGroup);
        } catch (error) {
            console.log("Error While Delete BloodGroup Con!! " + error.message);
            let obj = {
                statusCode: 400,

                message: "Error While Delete BloodGroup",
                result: null
            };
            res.send(obj);
        }
    ...
    uploadBloodGroup: async function (req, res) {
        try {
            ...
            if (req.files) {
                ...
                let result = null;
                if (ext == "csv") {
                    let uploadFileInServer = await RAR.App.Controller.BloodGroup.ConList.uploadFileInServer(req.files.fileData);
                    if (uploadFileInServer.statusCode == 400) {
                        error = true;
                        message = uploadFileInServer.message;
                    } else {
                        let readFileFromServer = await RAR.App.Controller.BloodGroup.ConList.readFileFromServer(uploadFileInServer.fileName);
                        ...
                        if (readFileFromServer.statusCode == 400) {
                            ...
                            let writeResponceFile = await RAR.App.Controller.BloodGroup.ConList.writeResponceFile(readFileFromServer.result, uploadFileInServer.fileName);
